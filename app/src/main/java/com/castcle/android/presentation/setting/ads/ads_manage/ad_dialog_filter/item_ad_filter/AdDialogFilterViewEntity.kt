...
data class AdDialogFilterViewEntity(
    val filter: AdFilterType = AdFilterType.All,
    val selected: Boolean = false,
    override val uniqueId: String = "${R.layout.item_wallet_dashboard_dialog_filter}"
) : CastcleViewEntity {

    override fun sameAs(isSameItem: Boolean, target: Any?) = if (isSameItem) {
        target?.cast<AdDialogFilterViewEntity>()?.uniqueId == uniqueId
    } else {
        target?.cast<AdDialogFilterViewEntity>() == this
    }
