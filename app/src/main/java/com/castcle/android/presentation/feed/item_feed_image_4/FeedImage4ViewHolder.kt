...
class FeedImage4ViewHolder(
    private val binding: ItemFeedImage4Binding,
    private val compositeDisposable: CompositeDisposable,
    private val listener: FeedListener,
    private val referenceType: CastType?,
) : CastcleViewHolder<FeedImage4ViewEntity>(binding.root), UserBarListener, ParticipateBarListener {

    override var item = FeedImage4ViewEntity()
    ...
    init {
        ...
        compositeDisposable += binding.ivImage4.onClick {
            listener.onImageClicked(item.cast.image.getOrNull(3) ?: ImageEntity())
        }
    ...
    override fun bind(bindItem: FeedImage4ViewEntity) {
        val marginBottom = if (
        ...
        binding.ivImage3.loadCenterCropWithRoundedCorners(
        ...
        binding.ivImage4.loadCenterCropWithRoundedCorners(
            thumbnailUrl = item.cast.image.getOrNull(3)?.thumbnail,
            url = item.cast.image.getOrNull(3)?.original,
            viewSizeDp = screenWidthPx().minus(dimenPx(com.intuit.sdp.R.dimen._28sdp)).div(2),
            enableTopLeft = false,
            enableTopRight = false,
            enableBottomLeft = false,
